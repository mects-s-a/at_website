...
export default function HeroCarousel() {
  ...
  return (
    <section className="relative overflow-hidden">
      {/* Alturas reduzidas de forma proporcional para maior compacidade */}
      <div className="relative h-[380px] sm:h-[460px] md:h-[500px] lg:h-[540px]">
        <AnimatePresence mode="wait">
          {slide.type === "hero" ? (
              ...
              <div className="relative max-w-7xl mx-auto px-4 sm:px-6 h-full flex items-center">
                <div className="grid lg:grid-cols-2 gap-8 items-center w-full">
                  <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.2 }}>
                    <p className="text-accent font-medium text-xs tracking-widest uppercase mb-3">Autoridade Tributária de Moçambique</p>
                    ...
                    <div className="flex flex-wrap gap-3">
                      ...
                      <a href="mailto:linhadocontribuinte@at.gov.mz" className="inline-flex items-center gap-2 bg-white/10 backdrop-blur px-5 py-2.5 rounded-xl font-semibold text-sm hover:bg-white/20 transition border border-white/20">
                        Fale Connosco
                      </a>
                    </div>
                  </motion.div>
                  ...
                  <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.6, delay: 0.4 }} className="hidden lg:grid grid-cols-2 gap-4">
                    <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-5 border border-white/10">
                      ...
                      <h3 className="font-semibold text-base mb-1">Nossa Missão</h3>
                    ...
                    <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-5 border border-white/10">
                      ...
                      <h3 className="font-semibold text-base mb-1">Visão</h3>
                    ...
                    <div className="col-span-2 bg-white/10 backdrop-blur-sm rounded-2xl p-4 border border-white/10">
                      <p className="text-accent font-semibold text-xs mb-1 uppercase tracking-wider">Valores</p>
            ...
            <motion.div
              ...
              exit={{ opacity: 0 }}
              transition={{ duration: 0.6 }}
              className={`absolute inset-0 ${slide.bgColor || "bg-black"}`}
            >
              {slide.url ? (
            ...
            {SLIDES.map((_, i) => (
              <button
                ...
                className={`rounded-full transition-all duration-300 ${
                  i === active ? "w-6 h-2 bg-accent" : "w-2 h-2 bg-white/50 hover:bg-white/80"
                }`}
              />
            ))}
          </div>
        ...
        <button
          onClick={prev}
          className="absolute left-3 sm:left-5 bottom-5 z-10 w-9 h-9 rounded-full bg-black/40 hover:bg-black/60 text-white flex items-center justify-center transition backdrop-blur-sm border border-white/10"
        ...
        <button
          onClick={next}
          className="absolute right-3 sm:right-5 bottom-5 z-10 w-9 h-9 rounded-full bg-black/40 hover:bg-black/60 text-white flex items-center justify-center transition backdrop-blur-sm border border-white/10"
