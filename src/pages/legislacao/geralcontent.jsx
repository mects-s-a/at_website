...
export default function GeralContent() {
  ...
  const activeCat = queryParams.get("cat") || "leis";
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3 bg-white p-3 rounded-xl border border-border shadow-sm">
        <div className="relative flex-1">
          ...
          <input type="text" placeholder="Pesquise na legislação geral..." className="w-full pl-10 pr-4 py-2 border border-border rounded-lg text-sm bg-muted/30 focus:outline-none focus:border-primary transition-colors" />
        </div>
      </div>
      {items.length > 0 ? (
