  ...
  return (
    <div className="w-full h-full flex flex-col gap-6">
      <div className="flex items-center justify-between border-b border-border pb-4 flex-wrap gap-3">
        ...
        <h1 className="text-2xl sm:text-3xl font-bold">{viewMode === "calendar" ? `${MONTH_NAMES[viewMonth]} ${viewYear}` : `Ano ${viewYear}`}</h1>
...
function FullMonthView({ year, month }) {
  ...
  return (
    <div className="grid grid-cols-7 gap-px bg-border border border-border shadow-sm">
      {WEEK_DAYS.map((d, i) => <div key={i} className="bg-muted p-4 text-center font-bold">{d}</div>)}
      {days.map((d, i) => (
