...
export default function FiscalContent() {
  ...
  // Retorna os dados correspondentes à subcategoria da barra lateral ou esvazia se não mapeado
  const itemsAApresentar = impostosLegis[activeCat] || [];
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3 bg-white p-3 rounded-xl border border-border shadow-sm">
        <div className="relative flex-1">
          ...
          <input
            type="text"
            placeholder="Pesquise por código, artigo ou palavra-chave..."
      ...
      {/* Grid de Conteúdo Controlado Dinamicamente */}
      {itemsAApresentar.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {itemsAApresentar.map((imp) => (
            <div key={imp.id} className="bg-white border border-border rounded-xl p-5 hover:shadow-md transition-all flex flex-col justify-between">
              ...
              <div className="flex items-center gap-2 pt-2 border-t border-border mt-auto">
                <button className="flex-1 flex items-center justify-center gap-1.5 text-xs font-medium border border-border py-2 px-3 rounded-lg hover:bg-muted transition-colors text-muted-foreground hover:text-foreground">
                  <Eye className="w-3.5 h-3.5" /> Ver Código
                </button>
                <button className="flex-1 flex items-center justify-center gap-1.5 text-xs font-medium bg-primary text-primary-foreground py-2 px-3 rounded-lg hover:opacity-90 transition-opacity">
        ...
        <div className="text-center py-12 border border-dashed rounded-xl text-muted-foreground text-sm">
          Nenhum documento disponível para esta categoria específica.
        </div>
      )}
    </div>
  );
