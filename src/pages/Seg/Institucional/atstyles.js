...
// --primary-dark                   → #0F3278
// --accent        hsl(152 55% 45%) → #33A068
// --gold                           → #C9A94B
// --foreground    hsl(215 30% 12%) → #151E2D
// --muted-bg      hsl(210 15% 94%) → #EEF1F5
...
const sobreATStyles = `
  ...
  .acc-item { border-bottom: 1px solid var(--sat-border); }
  ...
  .acc-trigger:hover { background: var(--sat-muted-bg); }
  .acc-trigger.open  { color: var(--sat-primary); background: var(--sat-muted-bg); }
  .acc-arrow { transition: transform 0.22s; font-size: 10px; color: var(--sat-muted-fg); flex-shrink: 0; }
  .acc-body  { padding: 0 22px 18px; background: var(--sat-card); font-size: 13.5px; color: var(--sat-muted-fg); line-height: 1.7; }
  ...
  .org-node {
    background: var(--sat-card); border: 1.5px solid var(--sat-border);
    border-radius: var(--sat-radius); padding: 14px 24px; text-align: center;
    box-shadow: var(--sat-shadow-sm); min-width: 230px; transition: border-color 0.2s;
  }
  .org-node:hover { border-color: var(--sat-gold); }
  .org-node.top { background: var(--sat-primary-dark); border-color: var(--sat-primary-dark); }
  .org-node.top strong { color: #fff; font-size: 14px; }
  .org-node.top span  { color: rgba(255,255,255,.72); }
  .org-node.mid { background: var(--sat-primary); border-color: var(--sat-primary); }
  ...
  .org-connector { width: 2px; height: 22px; background: var(--sat-border); }
  ...
  .org-hline { width: 100%; height: 2px; background: var(--sat-border); }

  /* ── FILTER CONTROLS ── */
  .filter-shell {
    display: flex; flex-direction: column; gap: 12px;
    align-items: center; justify-content: space-between;
    background: var(--sat-card); padding: 14px 20px;
    border-radius: var(--sat-radius); border: 1px solid var(--sat-border);
    box-shadow: var(--sat-shadow-sm); margin-bottom: 20px;
  }
  ...
  .search-input-wrap input {
    ...
    border: 1px solid var(--sat-border); border-radius: 8px;
    background: var(--sat-card); color: var(--sat-fg);
    outline: none; transition: border-color 0.2s, box-shadow 0.2s;
  }
  .search-input-wrap input:focus { border-color: var(--sat-primary); box-shadow: 0 0 0 3px rgba(27,79,171,.1); }

  .region-btn-group {
    display: flex; gap: 4px; background: var(--sat-muted-bg);
    padding: 3px; border-radius: 8px; border: 1px solid var(--sat-border); width: 100%;
  }

  .region-btn {
    flex: 1; text-align: center; padding: 7px 14px;
    font-family: 'Inter', sans-serif; font-size: 12px; font-weight: 500;
    border-radius: 6px; border: none; background: transparent;
    color: var(--sat-muted-fg); cursor: pointer; transition: all 0.15s; white-space: nowrap;
  }
  .region-btn.active { background: var(--sat-card); color: var(--sat-primary); box-shadow: var(--sat-shadow-sm); font-weight: 700; }
